...
object LoginHelper {
    fun login(
        ...
        val request = LoginRequest(usuario, password) // cuerpo JSON
                ...
                if (resp.isSuccessful() && resp.body() != null) {
                    try {
                        ...
                        if ("200" == json.optString("status")) {
                            /* --- Guardar datos esenciales en SharedPreferences --- */
                            ...
                            // guardamos todo lo que recibimos en el JSON "usuario"
                            ...
                            ed.putInt(
                                "tipo_comprobante_imprimir",
                                usuarioObj.optInt("tipo_comprobante_imprimir")
                            )
                            ed.putString("razon_social", usuarioObj.optString("razon_social"))
                            ed.putString("iibb", usuarioObj.optString("iibb"))
                            ed.putString(
                                "fecha_inicio_actividades",
                                usuarioObj.optString("fecha_inicio_actividades")
                            )
                            ...
                            ed.putString(
                                ...
                                empresaObj.optString("razon_social")
                            )
                            ed.putString("empresa_iibb", empresaObj.optString("iibb"))
                            ed.putString(
                                "empresa_fecha_inicio_actividades",
                                empresaObj.optString("fecha_inicio_actividades")
                            )
                            ed.putString("empresa_cert", empresaObj.optString("cert"))
                            ed.putString("empresa_key", empresaObj.optString("key"))
                            ed.putString(
                                "codigos_barras_id_long",
                                empresaObj.optString("codigos_barras_id_long")
                            )
                            ed.putInt(
                                "codigos_barras_long",
                                empresaObj.optInt("codigos_barras_long")
                            )
                            ed.putInt(
                                "codigos_barras_inicio",
                                empresaObj.optInt("codigos_barras_inicio")
                            )
                            ed.putInt(
                                "codigos_barras_payload_int",
                                empresaObj.optInt("codigos_barras_payload_int")
                            )
                            ed.putString(
                                "codigos_barras_payload_type",
                                empresaObj.optString("codigos_barras_payload_type")
                            )
                            ...
                            ed.putString(
                                ...
                                puntoVentaObj.optString("descripcion")
                            )
    ...
    /* Servicio Retrofit: envía JSON en el body */
